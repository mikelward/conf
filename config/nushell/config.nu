...
def --env setup-fnm [] {
    # Pick the default install dir the same way fnm's own installer does:
    # legacy ~/.fnm if present, else $XDG_DATA_HOME/fnm, else the macOS
    # Application Support dir, else ~/.local/share/fnm. FNM_PATH overrides.
    let fnm_path = (
        if ($env.FNM_PATH? | is-not-empty) {
            $env.FNM_PATH
        } else if ([$env.HOME ".fnm"] | path join | path exists) {
            [$env.HOME ".fnm"] | path join
        } else if ($env.XDG_DATA_HOME? | is-not-empty) {
            [$env.XDG_DATA_HOME "fnm"] | path join
        } else if ($nu.os-info.name == "macos") {
            [$env.HOME "Library" "Application Support" "fnm"] | path join
        } else {
            [$env.HOME ".local" "share" "fnm"] | path join
        }
    )
...
# First line of every file sync-tool-init writes, and the proof of
# ownership that lets it overwrite or remove one later.
const GENERATED_MARKER = "# generated by config.nu -- edits will be overwritten"
...
def sync-tool-init [tool: string, args: list<string>] {
    let file = $"generated-($tool).nu"
    let target = ((tool-autoload-dir) | path join $file)
    let existing = (if ($target | path exists) { open --raw $target } else { null })
    # Only ever touch a file this function wrote. The autoload directory
    # is the user's own -- a hand-written zoxide.nu belongs to them, and
    # deleting or overwriting it would silently destroy their config --
    # so generated files carry both a distinct name and a marker line,
    # and anything without the marker is left alone. The warning names
    # the file, not the path: $nu.home-dir can carry a real name.
    if $existing != null and (not ($existing | str starts-with $GENERATED_MARKER)) {
        warn $"($file) in the autoload directory wasn't generated by config.nu; leaving it alone"
        return
    }
...
# right fetch command per VCS (git/hg/jj), the per-VCS marker file to
# mtime-gate against, and how to detach the spawned process. Wired into
# hooks.env_change.PWD below so it only fires on real cd events; this
# function only owns the auth gate (skip when auth-info reports problems
# so the prompt's {behind} indicator still nags).
