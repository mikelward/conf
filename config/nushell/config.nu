...
def sync-tool-init [tool: string, args: list<string>] {
    let target = ((tool-autoload-dir) | path join $"generated-($tool).nu")
    ...
    # and anything without the marker is left alone.
    if ($target | path exists) and (not (open --raw $target | str starts-with $GENERATED_MARKER)) {
        warn $"($target) wasn't generated by config.nu; leaving it alone"
        return
    }
