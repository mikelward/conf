...
- **Restate every review comment in the reply, not just its count.** When you report a review, a comment, or a verdict in chat, give one bullet per comment, most blocking first: what it claims and where, in a sentence or two, and — where it needs a call from you rather than a fix — the decision and the options. A SHA, a comment count, and the gate's state say only whether something is blocking; the user decides how to respond from the finding itself, so a report without it just sends them to the thread to read it themselves. All of them in that one reply — this is the exception to *keep replies short*, which holds everywhere else: a bullet each stays short, and asking whether they are ready for the second finding costs a round trip per finding.
