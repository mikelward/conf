...
- **Shell support tiers.** The configs aren't held to one bar; a change lands by tier, and tier is capability/commitment, not the order features arrive. `shrc` alone spans several tiers by degradation (`$shell` guards plus the failsafe short-circuit), so tier is per-shell, not per-file.
  ...
  - `TODO:` decide how to handle ksh — `shrc` detects and configures it past the failsafe check but nothing tests it, so it sits outside the tiers for now. Make it a tested Tier 3 or mark it unsupported; tracked in `TODO.md`.
