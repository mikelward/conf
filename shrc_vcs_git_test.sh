...
# git_prompt_info returns branch and empty status for clean repo
result=$(cd "$_git_local" && git_prompt_info)
_expected_branch=$(cd "$_git_local" && git rev-parse --abbrev-ref HEAD)
branch=$(head -1 <<< "$result")
statuschars=$(sed -n '2p' <<< "$result")
assert_equal "git_prompt_info branch on clean repo" "$_expected_branch" "$branch"
assert_equal "git_prompt_info status on clean repo" "" "$statuschars"

# git_prompt_info returns branch and status chars with changes
(cd "$_git_local" && echo "prompt_info_test" > prompt_info_file.txt)
result=$(cd "$_git_local" && git_prompt_info)
branch=$(head -1 <<< "$result")
statuschars=$(sed -n '2p' <<< "$result")
assert_equal "git_prompt_info branch with changes" "$_expected_branch" "$branch"
assert_equal "git_prompt_info status with untracked" "??" "$statuschars"

# git_prompt_info with staged file
(cd "$_git_local" && git add prompt_info_file.txt)
result=$(cd "$_git_local" && git_prompt_info)
statuschars=$(sed -n '2p' <<< "$result")
assert_equal "git_prompt_info status with staged" "A" "$statuschars"

# git_prompt_info with multiple status types
(cd "$_git_local" && echo "another" > prompt_info_file2.txt)
result=$(cd "$_git_local" && git_prompt_info)
statuschars=$(sed -n '2p' <<< "$result")
assert_equal "git_prompt_info multiple statuses" "?? A" "$statuschars"

# git_prompt_info matches git_branch + status_chars
branch_direct=$(cd "$_git_local" && git_branch)
status_direct=$(cd "$_git_local" && status_chars)
branch=$(head -1 <<< "$result")
assert_equal "git_prompt_info branch matches git_branch" "$branch_direct" "$branch"
assert_equal "git_prompt_info status matches status_chars" "$status_direct" "$statuschars"

# clean up
(cd "$_git_local" && rm -f prompt_info_file2.txt && git commit -m "prompt_info test" >/dev/null 2>&1)

# git_prompt_info on a different branch
(cd "$_git_local" && git checkout -b prompt-info-branch >/dev/null 2>&1)
result=$(cd "$_git_local" && git_prompt_info)
branch=$(head -1 <<< "$result")
assert_equal "git_prompt_info on different branch" "prompt-info-branch" "$branch"
(cd "$_git_local" && git checkout "$_expected_branch" >/dev/null 2>&1 && git branch -D prompt-info-branch >/dev/null 2>&1)
