...
start_test "fish fish_command_not_found cds on trailing slash"
result="$(_fish_run '
    cd '"$_fish_autocd"'
    set -e CDPATH
...
start_test "fish fish_command_not_found no slash falls through"
result="$(_fish_run '
...
start_test "fish fish_command_not_found non-existent falls through"
result="$(_fish_run '
...
start_test "fish fish_command_not_found cds on ~/foo/"
...
start_test "fish resolve_cdpath_dir prints tilde-expanded path"
result="$(HOME=$_fish_autocd run_with_timeout 15 fish --no-config -c '
    source '"$_config"'
    set -e CDPATH
...
start_test "fish preserves and delegates to system hook"
...
start_test "fish re-sourcing preserves system hook idempotently"
result="$(HOME=$_testdir/fakehome run_with_timeout 15 fish --no-config -c '
    function fish_command_not_found
        echo "SYSTEM:$argv[1]"
    end
